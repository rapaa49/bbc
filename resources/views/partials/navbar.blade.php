    ...
    <div class="navbar-shell max-w-[1180px] mx-auto flex items-center justify-between gap-2.5 rounded-[36px] border border-white/20 bg-white/10 pl-4 pr-4 sm:pl-5 sm:pr-5 py-2 shadow-2xl shadow-black/15 backdrop-blur-3xl transition-all duration-500">
        ...
        <div class="hidden md:flex relative items-center justify-center flex-1 px-3">
            <div class="nav-items-group relative z-10 inline-flex max-w-[720px] items-center justify-center gap-2.5 px-1 py-1 rounded-full">
                ...
                <button type="button" data-url="{{ route('paket.index') }}" data-active="{{ request()->routeIs('paket.index') ? 'true' : 'false' }}" class="nav-item-button relative z-10 px-3.5 py-0.5 rounded-full text-[13px] font-semibold transition-colors duration-300 text-[#3a2a1a] hover:text-[#1f1410]">PAKET</button>
    ...
    <div id="mobileNavbarMenu" class="md:hidden mx-auto max-w-[1180px]">
        <div class="mobile-menu-inner rounded-[24px] border border-white/50 bg-white/95 backdrop-blur-2xl shadow-2xl shadow-black/10">
            <div class="flex flex-col p-3 gap-1">
                ...
                <a href="{{ route('paket.index') }}" class="group relative flex items-center justify-between px-4 py-3.5 rounded-xl text-[14px] font-semibold transition-all duration-300 {{ request()->routeIs('paket.index') ? 'bg-red-50 text-red-700' : 'text-[#3a2a1a] hover:bg-[#f8f5f0]' }}">
                    <span class="flex items-center gap-3"><i class="fas fa-boxes text-[18px] w-6 text-center transition-colors duration-300 {{ request()->routeIs('paket.index') ? 'text-red-600' : 'text-[#8b7355] group-hover:text-red-600' }}"></i> Paket</span>
                    @if(request()->routeIs('paket.index')) <i class="fas fa-chevron-right text-[11px] text-red-500"></i> @endif
                </a>
